...
/*   Updated: 2025/12/16 19:03:18 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	apply_affine_transform(t_point *point, double matrix[4][4])
{
	...
	point->view.x = round(m_x * matrix[0][0] + m_y * matrix[0][1] \
							+ m_z * matrix[0][2] + m_w * matrix[0][3]);
	point->view.y = round(m_x * matrix[1][0] + m_y * matrix[1][1] \
							+ m_z * matrix[1][2] + m_w * matrix[1][3]);
	point->view.z = round(m_x * matrix[2][0] + m_y * matrix[2][1] \
							+ m_z * matrix[2][2] + m_w * matrix[2][3]);
	point->view.w = round(m_x * matrix[3][0] + m_y * matrix[3][1] \
							+ m_z * matrix[3][2] + m_w * matrix[3][3]);
}
